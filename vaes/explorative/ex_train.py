...
for beta in [0.5, 1, 5, 20]:
    ...
    writer = SummaryWriter(f'../../runs/mnist/vae_beta_{beta}_{datetime.now().strftime("%Y%m%d-%H%M%S")}')

    prev_updates = 0
    for epoch in range(num_epochs):
        print(f'Epoch {epoch + 1}/{num_epochs} (beta={beta})')
        prev_updates = train(model, train_loader, optimizer, prev_updates, writer=writer)
        test(model, test_loader, prev_updates, writer=writer)

    writer.close()

    torch.save(model.state_dict(), f'../../vae_mnist_beta_{beta}.pth')
